...
class Client extends HttpClient {
    ...
    async connect(_notInitial) {
        if (!_notInitial) {
            if (!this._middleware.length) this.emit = (eventName, eventData) => this.events.emit(eventName, eventData, this);
            else {
                this.emit = (eventName, eventData) => {
                    ...
                    const next = (data) => {
                        ...
                        if (i >= this._middleware.length) return this.events.emit(eventName, eventData, this);
                        console.log(i);
                        this._middleware[i](eventName, eventData, next, this);
                    }
                    this._middleware[i](eventName, eventData, next, this);
                }
            }
        ...
        if (this._shardQueue.size) setTimeout(() => this.connect(true), 6000);
