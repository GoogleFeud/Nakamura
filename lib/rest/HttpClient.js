...
module.exports = class HTTPClient {
    ...
    _fetch(defUrl, data, detailed) {
        data.url = detailed ? defUrl + detailed:defUrl;
        return this._findRoute(defUrl).queueRequest(data);
    }
    ...
    sendToChannel(channelId, options) {
        ...
        if (options.file) {
            ...
            if (!options.filename) options.filename = "file.jpg";
            d.append("payload_json", JSON.stringify(data));
            d.append(options.filename, options.file, options.filename);
            data = d;
        }
        }
        return this._fetch(`/channels/${channelId}/`, {method: "post", body: data}, "messages")
    }
    ...
    async sendToUser(userId, options) {
        ...
        let dmChannel = _DmCache[userId];
        if (!dmChannel) {
            const c = await this._fetch("/users/@me/channels", {method: "post", body: {recipient_id: userId}});
            _DmCache[userId] = c.id;
            dmChannel = c.id;
        }
        return this._fetch(`/channels/${dmChannel}/`, {method: "post", body: data}, "messages");
    }
    ...
    createWebhook(channelId, options) {
        return this._fetch(`/channels/${channelId}/`, {method: "post", body: options}, "webhooks");
    }

    fetchChannelWebhooks(channelId) {
        return this._fetch(`/channels/${channelId}/`, {method: "get"}, "webhooks");
    }

    fetchGuildWebhooks(guildId) {
        return this._fetch(`/guilds/${guildId}/webhooks`, {method: "get"}, "webhooks");
    }

    fetchWebhook(webhookId) {
        return this._fetch(`/webhooks/${webhookId}`, {method: "get"});
    }

    fetchWebhookWithToken(webhookId, tokenId) {
        return this._fetch(`/webhooks/${webhookId}/${tokenId}`, {method: "get"});
    }

    updateWebhook(webhookId, update) {
        return this._fetch(`/webhooks/${webhookId}`, {method: "patch", body: update});
    }

    updateWebhookWithToken(webhookId, tokenId, update) {
        return this._fetch(`/webhooks/${webhookId}/${tokenId}`, {method: "patch", body: update});
    }

    deleteWebhook(webhookId) {
        return this._fetch(`/webhooks/${webhookId}`, {method: "delete"});
    }

    deleteWebhookWithToken(webhookId, tokenId) {
        return this._fetch(`/webhooks/${webhookId}/${tokenId}`, {method: "delete"});
    }

    sendWithWebhook(webhookId, webhookToken, options) {
        let data = {};
        if (typeof options === "string") data.content = options;
        else {
        Object.assign(data, options);
        ...
        return this._fetch(`/webhooks/${webhookId}/${webhookToken}`, {method: "post", body: data})
    }
