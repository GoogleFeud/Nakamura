    ...
    connect() {
        if (zlib) {
            this.inflate = new zlib.Inflate({
                ...
                to: pack.type === "json" ? "string":""
            });
        }
        return new Promise((resolve) => {
        this._ws = new ws(`${GATEWAY_URL}?v6&encoding=${pack.type}&compress=${(zlib) ? "zlib-stream":""}`)
        this._ws.on('open', _ => { this.debug(`Connection opened on shard ${this.shardId}`) })
        this._ws.on('message', msg => {
            if (zlib) {
                const l = msg.length;
                const flush = l >= 4 && msg[l - 4] === 0x00 && msg[l - 3] === 0x00 && msg[l - 2] === 0xff && msg[l - 1] === 0xff;
                this.inflate.push(msg, (l >= 4 && msg[l - 4] === 0x00 && msg[l - 3] === 0x00 && msg[l - 2] === 0xff && msg[l - 1] === 0xff) && zlib.Z_SYNC_FLUSH);
                if (!flush) return;
                msg = this.inflate.result;
            }
            msg = pack.unpack(msg);
            if (msg.s) {
                this._seq = msg.s
